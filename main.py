...
#week 8 code start here:
# create a database for the game
class DataBase:
    def __init__(self, Player1_name, Player2_name, Player1_type, Player2_type ):
        self.game_filename = "game.csv"
        ...
        #FIXME how to judge whether it is human or robots?
        # if Player1_name != "bot":
        # Player1_type = "Human"
        # else:
        # Player1_type = "bot"
    def player_type(self):
        if self.Player1_name != "bot":
            self.Player1_type = "Human"
        else:
            self.Player1_type = "bot"
        
        if self.Player2_name != "bot":
    ...
    def read_game(self):
        try:
            return pd.read_csv(self.game_filename)
    ...
    #game = read_game()
    def uploadData(self):#store the data of each game to csv file
        self.game.loc[len(self.game)] = {
            "GameID": len(self.game) + 1,
            "Player1": self.Player1_name,
            "Player1Type": self.Player1_type,
            "Player2": self.Player2_name,
            "Player2Type":self.Player2_type,
            "Winner": get_winner,
            "WinnerType": abb,
        }

        self.game.to_csv(self.filename)
